...
def main(cfg):
    ...
    if shutil.which("cdo") is None:
        raise EnvironmentError("CDO (Climate Data Operators) is not installed or not in your system's PATH. Please install CDO and ensure it's accessible.")

    for variable in cfg.variables:
        # Check if the input file exists
        input_file = os.path.join(cfg.data_dir, variable)
        if not os.path.isfile(input_file):
            logging.warning(f"Input file {input_file} not found. Skipping this variable.")
            continue

        logging.info(variable.split('_')[0].upper())
        intermediate_file = os.path.join(cfg.output_dir, f'{variable.split(".")[0]}_sellonlatbox_cropped.nc')
        output_file = os.path.join(cfg.output_dir, f'{variable.split(".")[0]}_time_sliced_cropped.nc')
        ...
        try:
            ...
            slice_time_command = f"cdo seldate,{cfg.start_date},{cfg.end_date} {intermediate_file} {output_file}"
